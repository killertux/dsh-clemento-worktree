...
import { WorktreeDropdown } from "./WorktreeDropdown.js";
const NS = 'worktree';
...
export async function apply(ctx) {
    ...
    const worktreeBySession = createSnapshotStore({});
    ...
    const queriedSessionIds = new Set();
    ...
    ctx.provide('worktreeWorkspace', {
        workspaceOf: (sessionId) => worktreeBySession.getSnapshot()[sessionId],
    });
    ...
    const syncWorktreeWorkspaces = () => {
        ...
        if (ids.length === syncedSessionIds.length
            && ids.every((id, index) => id === syncedSessionIds[index]))
            return;
    ...
    const injected = () => ({
        ...
        worktreeOf: async (sessionId) => {
            ...
            if (worktreeById.has(sessionId))
                return worktreeById.get(sessionId) ?? null;
        ...
        selectWorktree: async (worktreePath) => {
            ...
            const sessionRuntime = ctx.get('sessions');
