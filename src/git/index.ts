...
import type { GitCommandResult, GitWorktreeEntry } from './types.ts'
...
const collect: SubprocessCollect = { maxBytes: COLLECT_MAX_BYTES }
 ...
 */
export function siblingPath(workspacePath: string, branch: string): string {
...
export class WorktreeGit extends Service {
   ...
   */
  async listWorktrees(path: string): Promise<GitWorktreeEntry[]> {
    const result = await this.runGit(path, ['worktree', 'list', '--porcelain'])
    if (result.exitCode !== 0) {
      throw new GitCommandError(
        `cannot list worktrees of '${path}': git failed with exit ${result.exitCode}`,
   ...
   */
  async addWorktree(workspacePath: string, branch: string): Promise<GitWorktreeEntry> {
    ...
    if (result.exitCode !== 0) {
      throw new GitCommandError(
        `cannot create worktree on branch '${branch}' in '${workspacePath}': git failed with exit ${result.exitCode}`,
        ['git', '-C', workspacePath, ...argv],
        workspacePath,
        result.exitCode,
        result.stderr,
      )
   ...
   */
  async removeWorktree(workspacePath: string, worktreePath: string): Promise<void> {
    const argv = ['worktree', 'remove', '--force', worktreePath]
    const result = await this.runGit(workspacePath, argv)
    if (result.exitCode !== 0) {
      throw new GitCommandError(
        `cannot remove worktree '${worktreePath}': git failed with exit ${result.exitCode}`,
        ['git', '-C', workspacePath, ...argv],
        workspacePath,
        result.exitCode,
        result.stderr,
  ...
  private availableSiblingPath(workspacePath: string, branch: string): string {
  ...
  private async runGit(path: string, args: readonly string[]): Promise<GitCommandResult> {
    ...
    try {
      ...
      throw new GitCommandError(
        `failed to run git in '${path}': ${String(error)}`,
        argv,
        path,
        1,
        '',
